...
void Chiquadro(){
  ...
  TF1 *Pearson = new TF1("Pearson", "PearsonDist(x, 12)", 0, 90);
 
  TH1I *originale = new TH1I("originale", "Dati originali", 12, 0, 24);
  TH1D *chiquadro = new TH1D("chiquadro", "Distribuzione del #chi^{2}", 100, 0, 90);
  ...
  for(int i = 0; i < nesperimenti; i++){
    ...
    for(int j = 0; j < nbin; j++){
      datiesperimenti[j] = Poisson(dati[j], attesi[j]);
    }
